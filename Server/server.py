...
class Server:
    ...
    def fit(self, num_rounds: int, timeout: float | None) -> tuple[History, float]:
        ...
        if current_round == num_rounds:
            log(INFO, "")
            log(INFO, "Saving final global model...")
            arrays = parameters_to_ndarrays(self.parameters)
            s = self.preferences.sampled_training_nodes_per_round
            n = self.preferences.num_rounds
            l = self.preferences.seed
            numpy.savez(f"{self.preferences.fed_dir}/{n}_{s}_{l}/model.npz", *arrays)
            log(INFO, f"Final global model saved to '{self.preferences.fed_dir}//{n}_{s}_{l}/model.npz'")
    ...
    def test_round(
        ...
        if not client_instructions:
            log(INFO, "configure_test: no clients selected, skipping evaluation")
            return None
        log(
            INFO,
            "configure_test: strategy sampled %s clients (out of %s)",
        ...
        log(
            INFO,
            "aggregate_test: received %s results and %s failures",
            len(results),
            len(failures),
        )
    ...
    def fit_round(
        ...
        if server_round == self.preferences.num_rounds:
            log(INFO, "Saving client models...")
            for client_proxy, fit_res in results:
                client_params = fit_res.parameters
                s = self.preferences.sampled_training_nodes_per_round
                n = self.preferences.num_rounds
                l = self.preferences.seed
                dir_name = f"{self.preferences.fed_dir}/{n}_{s}_{l}"
                os.makedirs(dir_name, exist_ok=True)
                numpy.savez(
                    f"{dir_name}/client_{client_proxy.cid}_model.npz",
                    *parameters_to_ndarrays(client_params),
                )
            log(INFO, "Client models saved.")

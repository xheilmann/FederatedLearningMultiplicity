...
def calculate_rashomon_capacity(path: str, num_clients=2, project_name=None, run_name=None, dataset_name=None):
    ...
    for i in range(num_clients):
        ...
        for j in range(1,len(num_models)):
            ...
            if k ==0 and j==0:
                continue
            scores = ndarrays["all_sampling_test_scores"][:count,:,:]
    ...
    agg_metrics["rc_90_difpriv"] = [float(np.searchsorted(entry, 0.9, "left") * 0.001) for entry in rc_cum]
    agg_metrics["rc_50_difpriv"] = [float(np.searchsorted(entry, 0.5) * 0.001) for entry in rc_cum]
    for entry in rc_cum:
        entry[np.isnan(entry)] = 0
    agg_metrics["rc_difpriv"] = rc_cum



    rc = [[e for entry in rc_all for e in entry[i]] for i in range(len(rc_all[0]))]
    agg_metrics["rc_90"] = np.quantile(rc, 0.9, axis=1)
    agg_metrics["rc_50"] = np.quantile(rc, 0.5, axis=1)
